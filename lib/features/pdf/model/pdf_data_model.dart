...
class PdfDataModel {
  ...
  factory PdfDataModel.fromJson(Map<String, dynamic> json) {
    // Parse components
    ...
    return PdfDataModel(
      ...
      watchInformation: WatchInformation.fromJson(
        json['watch_information'] ?? {},
      ),
  ...
  CategoryAnalysis get categoryAnalysis {
    ...
    components.forEach((key, value) {
      ...
      items.add(
        CategoryItem(name: key, percentage: percentage, color: '#D4A574'),
      );
    });
...
String get formattedCreatedDate =>
    AppForMatters.formatDate(DateTime.parse(createdAt).toLocal());
...
class ComponentDetail {
  ...
  ComponentDetail({required this.matchScore, required this.observations});
...
class PdfFeatures {
  ...
  PdfFeatures({required this.includesStamps, required this.isBilingual});
...
class WatchImages {
  ...
  WatchImages({this.front, this.back, this.bracelet});
