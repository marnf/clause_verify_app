...
class AnalysisResultScreen extends StatelessWidget {
  ...
  // ✅ PDF button: unlock থাকলে আগের মতো, lock থাকলে lock icon ও আলাদা রঙ।
  // Lock থাকলেও চাপা যায়, চাপলে PDF কেনার sheet খোলে।
  Widget _buildViewPdfButton(AnalysisResultController controller) {
    final bool isBusy = controller.isGeneratingPdf.value;
    final bool isLocked = controller.isPdfLocked;
    final Color contentColor = isLocked ? const Color(0xFFB8860B) : Colors.black;

    return GestureDetector(
      onTap: isBusy ? null : controller.onPdfButtonTap,
      child: Container(
        width: double.infinity,
        padding: EdgeInsets.symmetric(vertical: 14.h),
        decoration: BoxDecoration(
          color: isLocked
              ...
              : (isBusy ? const Color(0xFFB8860B).withOpacity(0.5) : const Color(0xFFB8860B)),
          borderRadius: BorderRadius.circular(10),
          border: isLocked
              ? Border.all(color: const Color(0xFFB8860B).withOpacity(0.6), width: 1)
              : null,
        ),
        child: Center(
          child: isBusy
              ...
              : Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 8.w),
                    Text('viewPdfReport'.tr, style: TextStyle(color: contentColor, fontSize: 14.sp, fontWeight: FontWeight.w700)),
                  ],
                ),
        ),
      ),
